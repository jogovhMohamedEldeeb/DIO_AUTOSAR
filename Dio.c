 ...
 * @details 	Source file for ATMEGA32 Microcontroller - Dio Driver.
	...
	if (DIO_NOT_INITIALIZED == Dio_Status)
		{
			...
			for (uint8 i = 0; i < DIO_CONFIGURED_CHANNLES; ++i)
			{
				...
				else
				{
					...
					{
						case (PORTA_ID): Port_Ptr = &PORTA; break;
						case (PORTB_ID): Port_Ptr = &PORTB; break;
						case (PORTC_ID): Port_Ptr = &PORTC; break;
						case (PORTD_ID): Port_Ptr = &PORTD; break;
					}
					...
					{
						/*Set Pin Direction*/
						if(Dio_PortChannels->channels[i]->dir == OUTPUT)
						{
							CLEAR_BIT(*Port_Ptr, ChannelNum);
						}
						else if(Dio_PortChannels->channels[i]->dir == INPUT)
						{
 ...
 void Dio_SetupChannelDirection(Dio_PortType Port_Id, Dio_ChannelType Channel_Id, Dio_Direction dir)
{
 ...
 /* In this case there are no errors */
	if (FALSE == error)
	{
		...
		switch(Port_Id)
		{
			case (PORTA_ID): Port_Ptr = &DDRA; break;
			case (PORTB_ID): Port_Ptr = &DDRB; break;
			case (PORTC_ID): Port_Ptr = &DDRC; break;
			case (PORTD_ID): Port_Ptr = &DDRD; break;
		}
		if(Port_Ptr != NULL_PTR)
		{
			/*Set Pin Direction*/
...
void Dio_WriteChannel(Dio_PortType Port_Id, Dio_ChannelType Channel_Id,Dio_PinLevelType level)
{
	...
	/* In this case there are no error */
	if(FALSE == error)
	{
		uint8 ChannelNum = getChannelNum(Channel_Id);
		switch(Port_Id)
...
Dio_LevelType Dio_ReadChannel(Dio_PortType Port_Id, Dio_ChannelType Channel_Id)
{
	volatile uint8 *Port_Ptr = NULL_PTR;
	boolean error = FALSE;
	Dio_LevelType ret_value = STD_LOW;
...
Dio_PortLevelType Dio_ReadPort(Dio_PortType Port_Id)
{
	...
	if(FALSE == error)
	{
		switch(Port_Id)
		{
			case (PORTA_ID): Port_Ptr = &PINA; break;
			case (PORTB_ID): Port_Ptr = &PINB; break;
			case (PORTC_ID): Port_Ptr = &PINC; break;
			case (PORTD_ID): Port_Ptr = &PIND; break;
		}
	}
	return ret_value;
}
